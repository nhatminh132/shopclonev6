        ...
        foreach($CMSNT->get_list(" SELECT * FROM `connect_api` WHERE `status` = 1 AND `type` = 'API_9' ") as $website){
            ...
            if($data['error'] == 0){
                foreach($data['data'] as $category){
                    ...
                    if(!$CMSNT->get_row(" SELECT * FROM `categories` WHERE `id_api` = '".base64_encode($id_catrgory_api)."' AND `id_connect_api` = '".$website['id']."' ")){
                        $url_image = $CMSNT->site('favicon');
                        $CMSNT->insert('categories', [
                            'id_api'            => base64_encode($id_catrgory_api),
                    ...
                    foreach($category['products'] as $account){
                        ...
                        if(!$rowProduct = $CMSNT->get_row(" SELECT * FROM `products` WHERE `id_api` = '$id_product_api' AND `id_connect_api` = '".$website['id']."' ")){
                            // LẤY ID CATEGORY
                            $id_category = $CMSNT->get_row(" SELECT * FROM `categories` WHERE `id_api` = '".base64_encode($id_catrgory_api)."' AND `id_connect_api` = '".$website['id']."' ")['id'];
